...
class ArcReactor {
  ...
  /**
   * Performs Polymer powered analysis of the web component using `polymer-analyzer`.
   * The result of analysis is passes to the React generator.
   * @return {[type]} [description]
   */
  _analyze() {
    const analyzer = new ArcComponentsAnalyzer(this.opts.webComponent, this.logger);
  ...
  _generateComponents() {
    const builder = new Reactive(this.analysis, this.opts, this.dest, this.logger);
    try {
