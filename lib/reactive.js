...
class Reactive {
  ...
  buildComponents() {
    var names = this._listComponents();
    if (!names || !names.length) {
      throw new Error('Unknown list of components to reactify.');
    }
    this.logger.log('Building React wrappers for ' + names.length + ' web components');
    var modulesInfo = new Set();
    names.forEach(name => {
      let cmp = this.getComponentAnalysis(name);
      modulesInfo.add(this.computeInfo(cmp, name));
    });

    var result = this._computeComponentsContent(modulesInfo);
    if (this.opts.bundle) {
  ...
  _createComponentModules(iterator) {
    ...
    const componentPath = path.join(this.dest, componentName, componentName + '.js');
  ...
  _saveBundleComponent(content) {
    ...
    var location = path.join(this.dest, name);
   ...
   * @return {Object} Polymer element analysis result as defined in `polymer-analyzer`
   * module.
   */
  getComponentAnalysis(name) {
    var set = this.analysis.getFeatures({
      kind: 'element',
   ...
   */
  _listComponents() {
    var components = this.opts.reactComponents;
   ...
   * @return {Map} Map of each module's content. The key is React component name.
   */
  ...
  _bundleContent(map) {
    const it = map.values();
    var result = this._getReactHeader();
    while (true) {
      let item = it.next();
      if (item.done) {
        break;
  ...
  _getReactComponentDeclaration(data) {
    var body = this._getClassHeader(data.name);
  ...
  _getClassHeader(name) {
    var defaultStr = this.opts.bundle ? '' : 'default ';
    var result = `export ${defaultStr}class `;
  ...
  _getClassConstructor(component) {
    var result = '\tconstructor(props) {\n';
    result += '\t\tsuper(props);\n';
    // events local handlers
    // it also contains a list of "change" events
    var iterator = component.events.values();
    while (true) {
      let value = iterator.next();
      if (value.done) {
        break;
  ...
  _getClassUnmoutCallback(component) {
    var result = '\tcomponentWillUnmount() {\n';
    // events handling
    var iterator = component.events.values();
    while (true) {
      let value = iterator.next();
      ...
      let event = value.value;
      let eventName = this._computeLocalEventName(event.name);
      result += '\t\tthis._refElement.removeEventListener(\'' + event.name + '\',';
      result += ' this.' + eventName + ');\n';
    }
    result += '\t}\n\n';
    return result;
  ...
  _getClassUpdateCallback(properties) {
    var result = '\tcomponentDidUpdate(pp) {\n';
    var iterator = properties.values();
    while (true) {
      let value = iterator.next();
      if (value.done) {
        break;
      ...
      result += '\t\tif (pp.' + property.name + ' !== this.props.' + property.name + ') {\n';
      result += '\t\t\tthis._refElement.' + property.name + ' = ';
      result += 'this.props.' + property.name + ';\n';
      result += '\t\t}\n';
    }
  ...
  _getMethodsCallDeclarations(component) {
    var result = '';
    var iterator = component.properties.values();
    while (true) {
      let value = iterator.next();
      if (value.done) {
        break;
  ...
  _computeComponentIndexFile(name) {
    var result = `import ${name} from './${name}';`;
    result += '\n\n';
