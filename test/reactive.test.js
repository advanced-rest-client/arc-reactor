...
describe('Reactive test', function() {
  var analysis;
  before(() => {
    var analyzer = new ArcComponentsAnalyzer(bundleFile, logger);
  ...
  describe('getComponentAnalysis()', function() {
    ...
    it('Returns an object', function() {
      var result = builder.getComponentAnalysis('test-element');
  ...
  describe('Building React class declaration', function() {
    ...
    describe('_getClassMoutCallback()', function() {
      ...
      it('Attaches event listeners', function() {
        assert.notEqual(result.indexOf('.addEventListener(\'test-event-a\''), -1);
      });

      it('Attaches event listeners to the ref element', function() {
        assert.notEqual(result.indexOf('this._refElement.addEventListener('), -1);
      });

      it('Attaches property change listeners', function() {
        assert.notEqual(result.indexOf('.addEventListener(\'test2-changed\''), -1);
      });

      it('Sets properties', function() {
        assert.notEqual(result.indexOf('if (this.props.test1) {'), -1);
        assert.notEqual(result.indexOf('this._refElement.test1 = this.props.test1;'), -1);
      });
    });
    ...
    describe('_getClassUnmoutCallback()', function() {
      ...
      it('Detaches event listeners', function() {
        assert.notEqual(result.indexOf('.removeEventListener(\'test-event-a\''), -1);
      });

      it('Detatches event listeners to the ref element', function() {
        assert.notEqual(result.indexOf('this._refElement.removeEventListener('), -1);
      });
    });
    ...
    describe('_getClassUpdateCallback()', function() {
      ...
      it('Tests for property change', function() {
        assert.notEqual(result.indexOf('if (pp.test1 !== this.props.test1) {'), -1);
      });
      ...
      it('Does not contain readonly properties', function() {
        assert.equal(result.indexOf('this._refElement.test3 = this.props.test3;'), -1);
        assert.equal(result.indexOf('this._refElement.test4 = this.props.test4;'), -1);
      });
    });
    ...
    describe('_getEventsDeclarations()', function() {
      ...
      it('Calls handler', function() {
        assert.notEqual(result.indexOf('this.props.onTestEventA(e.detail, e);'), -1);
      });
    });
    ...
    describe('_getMethodsCallDeclarations()', function() {
      ...
      it('Declares arguments', function() {
        assert.notEqual(result.indexOf('publicMethodWithArguments(arg1, arg2) {'), -1);
      });

      it('Passes the arguments to element\'s function', function() {
        const index = result.indexOf('this._refElement.publicMethodWithArguments(arg1, arg2);');
    ...
    describe('_getClassRenderMethod()', function() {
      ...
      it('Sets the reference to the element', function() {
        const index = result.indexOf('ref={(element) => {this._refElement = element;}}');
        assert.notEqual(index, -1);
      });
    });
    ...
    describe('_computeComponentIndexFile()', function() {
      ...
      it('Should contain component import', function() {
        const result = builder._computeComponentIndexFile('TestName');
        assert.notEqual(result.indexOf('import TestName from \'./TestName\';'), -1);
      });
